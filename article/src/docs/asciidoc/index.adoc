...
The follwing section describe the four patterns that we'll use to implement a trivial application
...
The following diagram presents one of the many ways in which this pattern may be implemented; the dottec lines represent
passive links.
...
----
          ...
          \----*--------------/
               |         ^
    Read/Write |         | State change events
               |         |
               v         :
          /--------------*----\
          |                   |
...
=== PMVC: Presentation Model-View-Controller
...
to manipulate the *PresentationModel* directly, completly oblivious about a specific *View*. This leads to a much more
...
----
                           /-------------------\
                           |                   |
   Read/Write   +--------->| PresentationModel |<---------+ Read/Write
  (Databinding) |          |                   |          |
                |          \-------------------/          |
                |                                         |
                |                                         |
...
link:http://griffon-framework.org/[Griffon] framework. All applications use JavaFX as the toolkit of choice. Regardless
of the pattern all UIs rely on FXML to describe the *View* in a declarative approach as much as possible. The following
FXML file is thus shared by all applications

[source,xml,linenums,nowrap]
.org/example/sample.fxml
----
include::{mvc}/griffon-app/resources/org/example/sample.fxml[]
----

Input data is transformed by a simple service class, also shared by all applications

[source,java,linenums,nowrap]
.org.example.SampleService.java
----
include::{mvc}/griffon-app/services/org/example/SampleService.java[]
----
...
----
include::{mvc}/griffon-app/views/org/example/SampleView.java[]
----
...
----
include::{mvc}/griffon-app/models/org/example/SampleModel.java[]
----
<1> Notifies *View*
...
if followed correctly, will make the aconnection between *Controller* and *View*.

[source,java,linenums,nowrap]
.org.example.SampleController.java
----
include::{mvc}/griffon-app/controllers/org/example/SampleController.java[]
----
<1> Read input
<2> Transform data
<3> Set as output
...
.org.example.SampleView.java
----
include::{mvp}/griffon-app/views/org/example/SampleView.java[]
----
...
.org.example.SampleModel.java
----
include::{mvp}/griffon-app/models/org/example/SampleModel.java[]
----
...
----
include::{mvp}/griffon-app/controllers/org/example/SamplePresenter.java[]
----
<1> Read input
<2> Transform data
<3> Set as output
...
.Config.java
----
include::{mvp}/griffon-app/conf/Config.java[]
----
...
the *View* and the *ViewModel*. I opted to make use of the *Binder* in the *ViewModel*, rendering a *View* that's exactly
the same as in the previous two patterns.

[source,java,linenums,nowrap]
.org.example.SampleView.java
----
include::{mvvm}/griffon-app/views/org/example/SampleView.java[]
----
...
----
include::{mvvm}/griffon-app/controllers/org/example/SampleViewModel.java[]
----
<1> Read input
<2> Transform data
<3> Set as output
<4> Binds *Model* and *View*
...
.Config.java
----
include::{mvvm}/griffon-app/conf/Config.java[]
----
...
.org.example.SampleView.java
----
include::{pmvc}/griffon-app/views/org/example/SampleView.java[]
----
<1> Connects user events to *Controller*
...
.org.example.SampleModel.java
----
include::{pmvc}/griffon-app/models/org/example/SampleModel.java[]
----
...
.org.example.SampleController.java
----
include::{pmvc}/griffon-app/controllers/org/example/SampleController.java[]
----
<1> Read input
<2> Transform data
<3> Set as output
...
The code for all applications is readily available on GitHub at link:https://github.com/aalmiray/griffon-patterns[aalmiray/griffon-patterns].
